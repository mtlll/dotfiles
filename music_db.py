...
def _query_from_mpd_search_parameters(parameters, mapping, return_uris=False):
    query = {}
    uris = []
    ...
    while parameters:
        ...
        if value.strip():
            if return_uris and field == 'uri':
	        map(lambda x: uris.append(x.strip()), value.split(','))
            else:
...
def find(context, *args):
    ...
    *ncmpcpp:*
    ...
    """
    try:
        (query, uris) = _query_from_mpd_search_parameters(args, _SEARCH_MAPPING, True)
    except ValueError:
        return

    with deprecation.ignore('core.library.search:empty_query'):
        results = context.core.library.search(query=query, uris=uris, exact=True).get()
    result_tracks = []
...
def search(context, *args):
    ...
    *ncmpcpp:*
    ...
    try:
        ...
        return
    with deprecation.ignore('core.library.search:empty_query'):
        results = context.core.library.search(query, uris).get()
